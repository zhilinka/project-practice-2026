# Руководство по созданию Telegram-бота для конвертации валют
...
def build_app(token: str) -> Application:
    ...
    app = (
        ...
        .token(token)
        .request(request)
        .post_init(post_init)   # устанавливает меню команд после запуска
        .build()
    )
    ...
    conv = ConversationHandler(
        ...
        allow_reentry=True,   # позволяет снова вызвать /convert не завершая диалог
    )
...
| `get_rate()` | Извлекает курс конкретной пары из словаря rates | `Optional[float]` |
| `format_amount()` | Форматирует сумму: `22625.5` → `"22,625.50"` | `str` |
| `format_rate()` | Форматирует курс: `90.5` → `"90.5000"` | `str` |
| `post_init()` | Устанавливает меню команд в Telegram после запуска | `None` (async) |
...
В качестве творческой модификации в бот добавлена расширенная версия
команды `/rate` с необязательным третьим аргументом — суммой для
конвертации.
...
- Форматирование через `format_amount()` — использует `f"{x:,.2f}"`, добавляющий разделители тысяч
...
Найдите бота в Telegram и напишите `/start`. Меню команд (`/`) появится
автоматически — оно устанавливается через `post_init()` при старте.

Для остановки нажмите **Ctrl+C**: PTB корректно завершит polling и
закроет соединения.
